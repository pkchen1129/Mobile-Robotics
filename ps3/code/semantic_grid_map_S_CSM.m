...
%                 obj.map.mean(i,:) = obj.map.alpha(i,:) / ( sum(obj.map.alpha(i,:)) * ones(1,obj.num_classes+1) );
                obj.map.mean(i,:) = obj.map.alpha(i,:) / N; %1*7  
                max_mean = max(obj.map.mean(i,:));  %1*1
                obj.map.variance(i) = max_mean * (1 - max_mean) / (N + 1); % (16700 * 1 in total) 
                %mean = (16700 * 7 in total)
                %variance = (16700 * 1 in total)
                % -----------------------------------------------
            end
        end
        ...
        function S_CSM(obj, z, i)
            ...
            [bearing_min, k] = min(bearing_diff);
                ...
                elseif obj.m_i.range <  z(k,1) && z(k,1) < obj.z_max %renew the free space
