...
void Batch2D(){
    ...
    // output file
    string outfile = "../dataset/out.txt";
...
void Incremental2D(){
    ...
    for(int i = 1 ; i < pair_vertex.first.size() ; i++){
        ...
        // Add the current node to init value using result.at(i) + odometry
        pair_vertex.first[i];

        for(int j = 1 ; j < pair_edge.first.size() ; j++){ // A vector
            ...
            if(pair_edge.first[j]->first.second == i){ // id2 == current node
                ...
                graph.push_back(pair_edge.second[j]); // Add 
            }
        }
        ...
        Values currentEstimate = isam.calculateEstimate(); // What format would this be?
    }
